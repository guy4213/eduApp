...
const toggleCardExpansion = (cardId: string) => {
  ...
  const fetchReportStatuses = async (courseInstanceIds: string[]): Promise<Map<string, {isCompleted: boolean, isLessonOk: boolean, reportId?: string}>> => {
    ...
    try {
      console.log('[DEBUG] Fetching report statuses for course instances:', courseInstanceIds);
        ...
        instances.forEach((instance: any) => {
          ...
          keys.forEach(key => {
            if (key) {
              ...
              if (!existing || new Date(report.created_at) > new Date(existing.reportId)) {
                statusMap.set(key, {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('CourseAssignments: Reported lesson instance changed:', payload);
          debouncedRefresh();
        }
      )
      .subscribe();
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'lessonReportUpdated') {
        console.log('CourseAssignments: Lesson report updated via storage, refreshing...');
        statusCache.clear();
        fetchAssignments();
      }
    };
    ...
    const handleCustomEvent = () => {
      console.log('CourseAssignments: Custom lesson report event, refreshing...');
      statusCache.clear();
      fetchAssignments();
    };
      ...
      <CardHeader className="text-white rounded-t-lg bg-gradient-to-r from-blue-600 to-blue-700">
        <div className="flex justify-between items-start">
          <div>
            ...
            <div className="flex items-center gap-2 mb-2">
              <CardTitle className="text-2xl text-white">{assignment.name}</CardTitle>
              <Badge className="bg-green-500/20 text-green-100 border-green-300/30">
                מוקצה
              </Badge>
            </div>
      ...
      <CardContent className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          ...
          <div className={`${getSchoolTypeColors(assignment.school_type).bg} p-4 rounded-lg border ${getSchoolTypeColors(assignment.school_type).border}`}>
            ...
            <span className="text-lg font-bold text-gray-900">{getSchoolTypeDisplayName(assignment.school_type)}</span>
          </div>
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <span className="text-lg font-bold text-gray-900">{assignment.grade_level}</span>
          </div>
          ...
          <div className="bg-green-50 p-4 rounded-lg">
            ...
            <span className="text-lg font-bold text-gray-900">{assignment.max_participants}</span>
          </div>
          ...
          <div className="bg-purple-50 p-4 rounded-lg">
            ...
            <span className="text-lg font-bold text-gray-900">{assignment.lesson_count}</span>
          </div>
          ...
          <div className="bg-orange-50 p-4 rounded-lg">
            ...
            <span className="text-lg font-bold text-gray-900">₪{assignment.price_for_customer}</span>
          </div>
        </div>
          ...
          <div className="space-y-6">
            {Object.entries(groupTasksByLesson(assignment.tasks)).map(([lessonNumber, tasks]) => {
              ...
              const lessonStatus = (() => {
                ...
                if (!report?.isReported) return { text: "📋 טרם דווח", color: "bg-gray-500" };
                if (report.isCompleted === false) return { text: "❌ לא התקיים", color: "bg-orange-500 text-white" };
                if (report.isCompleted && report.isLessonOk === false) return { text: "⚠️ לא התנהל כשורה", color: "bg-red-500 text-white" };
                return { text: "✅ דווח והתקיים", color: "bg-green-500 text-white" };
              })();
              ...
              return (
                <div key={lessonNumber} className="bg-gray-50 rounded-lg p-4">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <Badge className={`${lessonStatus.color}`}>{lessonStatus.text}</Badge>
                  </div>
